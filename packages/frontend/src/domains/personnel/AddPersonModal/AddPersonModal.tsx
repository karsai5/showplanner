...
export const AddPersonModal: React.FC<{
	...
	showId
		...
		const { data, isLoading, isError } = useQuery(["unassigned-people", showId], () =>
		...
		const mutation = useMutation<unknown, Error, string | undefined>({
			...
			onSuccess: () => {
				...
				queryClient.invalidateQueries({ queryKey: ["unassigned-people", showId] });
		...
		return <>
			<button className="btn btn-sm" onClick={() => open()}>Add Person</button>
