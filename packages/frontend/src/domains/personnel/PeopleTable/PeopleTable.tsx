...
export const PeopleTable: React.FC<{
  ...
  if (data && data.people) {
    return <PurePeopleTable
      people={data.people}
      showPrivateDetails={hasPermission(showPermission(showId, PERMISSION.personnelPrivateDetails))}
    />
  }
  return null;
};
...
const baseColumns = [
  columnHelper.accessor(row => row, {
    id: 'name',
    cell: info => <PersonDisplayName className="text-nowrap" person={info.getValue()} />,
    header: 'Name',
  }),
]
const privateDetailsColumns = [
  columnHelper.accessor(row => row._private?.phone, {
    id: 'phone',
    cell: info => <a className="link text-nowrap" href={`tel:${info.getValue()}`}>{info.getValue()}</a>,
    header: 'Phone',
  }),
  columnHelper.accessor(row => row._private?.email, {
    id: 'email',
    cell: info => <a className="link text-nowrap" href={`mailto:${info.getValue()}`}>{info.getValue()}</a>,
    header: 'Email',
  }),
...
export const PurePeopleTable: FC<{ people: PersonSummaryDTO[], showPrivateDetails?: boolean }> = ({ people, showPrivateDetails }) => {
  const table = useReactTable({
    data: people,
    columns: [...baseColumns, ...(showPrivateDetails ? privateDetailsColumns : [])],
