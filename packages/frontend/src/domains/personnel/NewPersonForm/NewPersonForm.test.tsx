...
const fillInDefaultValues = async (user: UserEvent) => {
  return act(async () => {
    ...
    await user.type(
      screen.getByPlaceholderText(/Preferred name/i),
      preferredName,
    );
    ...
    await user.type(
      screen.getByLabelText(/RElationship to emergency contact/i),
      emergencyRelation,
    );
  });
};
  ...
  it('it should allow the user to fill in their own pronoun', async () => {
    ...
    await act(async () => {
      await userEvent.selectOptions(
        screen.getByRole('combobox', { name: 'Pronoun' }),
        'Other',
      );
    ...
    await waitFor(() =>
      expect(api.mePost).toBeCalledWith({
        personalDetails: {
          allergies: 'None',
          dob: '1999-09-09',
