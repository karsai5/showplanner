...
func (d *Database) GetShowsForUser(id uuid.UUID) ([]Show, error) {
	person := Person{}
	res := db.Preload("Shows.Events").Preload("Shows.Image").First(&person, id)
	return person.Shows, res.Error
}

func (d *Database) GetEventsWithAvailabilityAndAssignmentsForUser(showId uint, userId uuid.UUID) ([]Event, error) {
	var events []Event
